...
// We want to replace from startMarker until the return statement.
// But we need to keep the closing braces of MainDashboardPanel.
// MainDashboardPanel ends with:
//     </div>
// );

const panelEndMarker = '        </div>\n    );';
const panelEndIdx = c.lastIndexOf(panelEndMarker, endIdx);

if (panelEndIdx === -1) {
    console.error('Panel end marker not found');
    process.exit(1);
}

const before = c.substring(0, startIdx + startMarker.length);
const after = c.substring(panelEndIdx);
...
console.log('Successfully reconstructed MainDashboardPanel');
