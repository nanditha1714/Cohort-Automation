...
const startMarker = '<div id="report-content" className="flex-1">';
const startIdx = c.indexOf(startMarker);

// Find the end of the MainDashboardPanel JSX
const panelEndMarker = '        </div>\n    );';
const panelEndIdx = c.indexOf(panelEndMarker, startIdx);
...
// If literal panelEndMarker failed, try a regex
let endPos = panelEndIdx;
if (endPos === -1) {
    const regex = /\s+<\/div>\s+\);\s+const NavigationHeader = \(/;
    ...
    if (match) {
        ...
        if (returnIdx !== -1) {
            ...
            const lastDiv = c.lastIndexOf('</div>', returnIdx);
            if (lastDiv !== -1) {
                endPos = lastDiv - 8; // Adjust for indentation
            }
        }
    }
}
...
if (endPos === -1) {
    console.error('Could not determine end of block');
    process.exit(1);
}

const before = c.substring(0, startIdx + startMarker.length);
const after = c.substring(endPos);
...
console.log('Successfully reconstructed MainDashboardPanel v2');
